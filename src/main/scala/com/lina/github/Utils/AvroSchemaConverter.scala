...
import org.apache.avro.LogicalTypes.{Date, TimestampMicros, TimestampMillis, Decimal}
...
object AvroSchemaConverter {
  ...
  /**
    ...
    */
  /*Can be used from SchemaConverters also*/
    ...
    def createConverter(avroSchema: Schema,
      ...
      (sqlType, avroType) match {
        ...
        case (BinaryType, FIXED) =>
          ...
          /*Custom code added to handle TimestampType*/
        case (TimestampType,LONG) =>
          avroSchema.getLogicalType match {
            case _: TimestampMillis | _: TimestampMicros => (item: AnyRef) =>{
              if (item == null) {
                null.asInstanceOf[java.sql.Timestamp]
              } else {
                new java.sql.Timestamp(item.asInstanceOf[Long])
              }
            }
            case _ => (item: AnyRef) =>{
              item
          ...
          /*End of Custom code for handling Timestamp Type*/
  ...
  /**
    ...
    */
  private def getNewRecordNamespace(
